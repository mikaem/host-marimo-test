...
def _(c, jnp, loss_fn, opt_lbfgs, trainer, u, x, xi):
    ...
    trainer.train(opt_lbfgs, 5000, epoch_print=1000, abs_limit_loss=1e-10, print_final_loss=True)
    trainer.step(u.module)
...
def _(
    ...
    for _step in range(1, Nsteps):
        ...
        trainer.train(opt_lbfgs, 2000, epoch_print=500, abs_limit_loss=1e-10, print_final_loss=True)
        unm1 = un
        un = u(xi)
        trainer.step(u.module)
    x1 = mesh.get_points(1000, domain='all', kind='uniform')
    uej_1 = lambdify(x, ue.subs(t, dt * Nsteps))(x1[:, 0])
    print('Error at final time step:', jnp.linalg.norm(u(x1) - uej_1) / jnp.sqrt(uej_1.shape[0]))
    return (Nsteps,)
...
def _(Nsteps, dt, exact, jnp, plt, uj):
    uej_2 = jnp.array(exact)
    plt.contourf((uej_2 - uj).T, extent=[0, Nsteps * dt, 0, jnp.pi], levels=100, cmap='jet')
